...
(6, 1, '24', NULL, NULL, '超棒的初體驗！！', '講解很仔細，教練時間安排的很充實，照片很棒教練還會專業指導網美式擺位，隨旁有多個教練也會注意大家的安全，即使不小心被浪拍上岸教練也會馬上追上把你拉回來，結束時還有小點心跟飲品補充體力，跟精美小禮物，真的很貼心！明年夏天還想再去玩！', 5, NULL, '2021-08-03 14:02:25', '2021-08-03 14:02:25'),
(7, 1, '24', NULL, NULL, '好玩', '第一次玩很好玩 與家人美好的回憶 超棒的', 5, NULL, '2021-08-03 14:02:25', '2021-08-03 14:02:25'),
...
(27, 4, '珊瑚海秘境浮潛', '浮潛subtitle', '浮潛article', 1000, '2021-08-10', '2021-07-13 09:57:42', NULL, 1, 123, 15, NULL, 1, 5, 8, 6, '001.jpg', '15:00'),
...
(33, 5, '森林冥想瑜珈', '冥想瑜珈subtitle	', '冥想瑜珈article', 300, '2021-08-09', '2021-07-13 10:15:41', NULL, 1, 412, 128, NULL, 2, 8, 2, 5, '002.jpg', '15:00'),
...
(37, 5, '野炊手作料理', '手作料理subtitle	', '手作料理article', 700, '2021-08-09', '2021-07-13 10:20:55', NULL, 1, 230, 45, NULL, 2, 7, 5, 2, '005.jpg', '15:00'),
...
(2, 'crew', '1234', '美琳', '林', 'crew@gmail.com', '0987654321', 2, '1976-12-07', 974042, '山嶺18號', 'B2C', 32, 1, '2021-07-16 14:08:11', '2021-08-09 22:36:42', 5, 5, 5, NULL),
...
--
-- 資料表索引 `ordered_detail`
--
ALTER TABLE `ordered_detail`
  ADD UNIQUE KEY `o_id` (`o_id`,`i_id`,`ship_date`),
  ADD KEY `i_id` (`i_id`);

--
-- 資料表索引 `ordered_pay`
--
ALTER TABLE `ordered_pay`
  ADD KEY `o_id` (`o_id`);
...
ALTER TABLE `users_likes`
  ADD UNIQUE KEY `u_id_2` (`u_id`,`i_id`),
  ADD KEY `u_id` (`u_id`);
...
--
ALTER TABLE `discuss`
  MODIFY `d_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `gallery`
--
ALTER TABLE `gallery`
  MODIFY `g_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=34;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `items`
--
ALTER TABLE `items`
  MODIFY `i_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=59;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `items_cat`
--
ALTER TABLE `items_cat`
  MODIFY `ic_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=33;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `ordered`
--
ALTER TABLE `ordered`
  MODIFY `o_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=95;
...
--
ALTER TABLE `users`
  MODIFY `u_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
...
--
ALTER TABLE `discuss`
  ADD CONSTRAINT `discuss_ibfk_1` FOREIGN KEY (`u_id`) REFERENCES `users` (`u_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `gallery`
--
ALTER TABLE `gallery`
  ADD CONSTRAINT `gallery_ibfk_1` FOREIGN KEY (`u_id`) REFERENCES `users` (`u_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `items_cat`
--
ALTER TABLE `items_cat`
  ADD CONSTRAINT `items_cat_ibfk_1` FOREIGN KEY (`c_id`) REFERENCES `category` (`c_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `items_cat_ibfk_2` FOREIGN KEY (`i_id`) REFERENCES `items` (`i_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `ordered`
--
ALTER TABLE `ordered`
  ADD CONSTRAINT `ordered_ibfk_1` FOREIGN KEY (`u_id`) REFERENCES `users` (`u_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `ordered_detail`
--
ALTER TABLE `ordered_detail`
  ADD CONSTRAINT `ordered_detail_ibfk_1` FOREIGN KEY (`i_id`) REFERENCES `items` (`i_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `ordered_detail_ibfk_2` FOREIGN KEY (`o_id`) REFERENCES `ordered` (`o_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `ordered_pay`
--
ALTER TABLE `ordered_pay`
  ADD CONSTRAINT `ordered_pay_ibfk_1` FOREIGN KEY (`o_id`) REFERENCES `ordered` (`o_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `users_likes`
--
ALTER TABLE `users_likes`
  ADD CONSTRAINT `users_likes_ibfk_1` FOREIGN KEY (`u_id`) REFERENCES `users` (`u_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `users_points`
--
ALTER TABLE `users_points`
  ADD CONSTRAINT `users_points_ibfk_1` FOREIGN KEY (`u_id`) REFERENCES `users` (`u_id`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
